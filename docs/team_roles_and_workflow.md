...
10. **代管与降本授权（新增，2026-09-15 用户指令）**：①集成机器人(integrator agent,Flash)自动完成: 提交→worktree 测试→合并→全量回归→盲审(Kimi/Sonnet)自动归档+分级;失败/冲突/复查 Fail 一律留人;②顶层日常编排由 Flash 版 `orchestrator` 代理承担;DeepSeek V4 Pro 保留三职: 评审冲突仲裁、架构/DAG 级重定向、向用户综合汇报;③自动化盲审节流 ≤2 次/30 分钟,成本熔断照旧。
