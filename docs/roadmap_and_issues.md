...
├── 🚩 Milestone 3: 视觉模型推理管线 (PaddleOCR 商店)
...
- **验收标准**: 自动采样与手动单帧快照响应延迟 $< 50\text{ms}$，图片清晰无拉伸变形，可直接作为后续 OCR 标注训练样本。
...
### 🚩 Milestone 2: 数据集标注、预处理与 4x7 棋盘/商店 ROI 定位
- **Issue #7**: 对局采样帧数据清洗、图像去重（感知哈希）与标注格式转换
...
#### Issue #5: [Vision] 棋盘 4x7 网格、弈子星级与装备视觉识别
- **标签 (Labels)**: `model`, `vision`, `M2`
- **任务清单**:
  - [ ] 实现小目标（星级、装备图标）识别后处理与 NMS (非极大值抑制)
