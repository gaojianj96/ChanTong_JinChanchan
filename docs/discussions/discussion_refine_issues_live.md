...
3. **数据可用**：输出的帧序列可直接用于后续视觉模型训练（如 YOLO 标注工具可读取）。
...
- **核心 Issue**：
  - 基于 OpenCvSharp 的 DNN 模块加载 ONNX 模型（YOLOv8 或自定义）。
  - 棋子检测、装备识别、羁绊状态识别。
  - 离线推理引擎（对帧序列批量分析）。
...
| M3 | 棋子/装备/状态识别 | 3–4 周 | YOLO/模板匹配检测器，OCR 数字 | 在 M1 数据上 mAP > 0.85 |
