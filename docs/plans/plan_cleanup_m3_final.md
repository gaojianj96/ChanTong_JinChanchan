...
## 5. 执行状态 (2026-09-12 更新)

| 任务 | 状态 | 备注 |
|---|---|---|
| A1/A2/A3 | ✅ | 门禁通过;校准经用户 R4 目检二次修正;A3 转 smoke |
| B1-B5 + C0b | ✅ | 全部双盲评审合并,commit 落库 |
| C0/C1a/C2 | ✅ | 同上 |
| C4.5/C4/C3a/C3b | ✅ | probe/go-no-go 实机跑通(30fps 口径 Go) |
| C5/C6 | ✅ | 解码/字典层真实化;实机识别准确率待 R5(训练另开会话进行) |
| R1 权重 | ✅ | yolo26n.onnx(SHA256 2E947B78…) |
| R2 延迟 | ✅ | 30fps 口径(用户裁定),DirectML 27.2ms |
| R3 多DPI | ⏸ | 用户决定单屏固定,跳过 |
| R4 Golden | ✅ | 用户目检全对齐 |
| R5 准确率 | ⏳ | 弈铲专属模型训练进行中(独立会话) |

**下一步**: R5 模型回传 → 实机识别准确率报告 → M3 关闭。
